...
string recvFileName()
{
	...
	if(msgrcv(msqid, &fileMsg, sizeof(fileMsg) - sizeof(long), FILE_NAME_TRANSFER_TYPE, 0) < 0)
	{
		perror("msgrcv");
        exit(-1);
	}
	/* TODO: return the received file name */
		fileName = string(fileMsg.fileName);
        return fileName;
}
 /**
...
unsigned long mainLoop(const char* fileName)
{
	...
	{	
        ...
        if (msgsnd(msqid, &ackMsg, sizeof(ackMsg) - sizeof(long), 0) < 0) {
            perror("msgsnd");
            fclose(fp);
            exit(-1);
        }
